...
class FPA():
            ...
            for i in range(self.populationSize):
                ...
                while temp[0]<1 or temp[0]>= self.map.size[0] or temp[1]<1 or temp[1]>= self.map.size[1]:
                    if(random.random()>self.probability):
                        ...
                        temp=temp.round()
                        nextPopulation[i]=temp.astype(int)
                    else:
    ...
    def UpdateQ(self):
        ...
        self.Q[pos][self.a]=(1-self.updateAlpha)*self.Q[pos][self.a]+self.updateAlpha*(self.r+self.gamma*max(self.Q[self.nextState[0]+(self.nextState[1]-1)*20 -1]))
